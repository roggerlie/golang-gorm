...
func TestCtx(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/hello?name=Nirwan", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Nirwan", string(bytes))
...
func TestHttpRequest(t *testing.T) {
	...
	request.Header.Set("firstname", "Nirwan")
	request.AddCookie(&http.Cookie{Name: "lastname", Value: "Yakub"})
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Nirwan Yakub", string(bytes))
}
...
func TestRouteParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/users/nirwan/orders/10", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Get Order 10 From User nirwan", string(bytes))
}
...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=Nirwan")
...
func TestRequestBody(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Nirwan", "password":"rahasia"}`)
...
func TestBodyParserJSON(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Nirwan", "password":"rahasia","name":"Nirwan Yakub"}`)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Register Success Nirwan", string(bytes))
}
...
<RegisterRequest>
<username>Nirwan</username>
<password>rahasia</password>
<name>Nirwan Yakub</name>
</RegisterRequest>
`)
	request := httptest.NewRequest("POST", "/register", body)
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"Nirwan Yakub","username":"nirwan"}`, string(bytes))
}
...
func TestDownloadFile(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "this is sample file to upload", string(bytes))
}
