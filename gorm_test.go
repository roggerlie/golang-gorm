...
func OpenConnection() *gorm.DB {
	dialect := "root@tcp(127.0.0.1:3306)/belajar_golang_gorm?charset=utf8mb4&parseTime=True&loc=Local"
...
func TestExecuteSQL(t *testing.T) {
	err := db.Exec("insert into sample(id, name) values(?,?)", "1", "Nirwan").Error
	assert.Nil(t, err)
	...
	err = db.Exec("insert into sample(id, name) values(?,?)", "4", "Luli").Error
	assert.Nil(t, err)
}
...
func TestRawSQL(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "Nirwan", sample.Name)
...
func TestOrderLimitOffset(t *testing.T) {
	var users []User
	err := db.Order("id asc, first_name desc").Limit(5).Find(&users).Error
	assert.Nil(t, err)
...
func TestUpdate(t *testing.T) {
	...
	user.Name.FirstName = "nirwan"
	user.Name.MiddleName = ""
	user.Name.LastName = "yakub"
...
func TestPreloadManyToMany(t *testing.T) {
	var product Product
	err := db.Preload("LikeByUsers").Take(&product, "id = ?", "P001").Error
	assert.Nil(t, err)
